...
def get_delta_update(simulation_id, current_state):
    """현재 상태를 그대로 전달 (실시간 모니터링용)"""
    # 실시간 업데이트에서는 항상 전체 데이터를 전송
    # (Delta 압축은 대용량 데이터에서만 필요)
    delta_buffer[simulation_id] = current_state
    return {"type": "full_update", "data": current_state}
...
def handle_disconnect():
    ...
    if session_id in client_subscriptions:
        ...
        logger.info(
            f"Client disconnected from simulation {simulation_id}: {session_id}"
        )
    else:
...
def handle_unsubscribe(data):
    ...
    if session_id in client_subscriptions:
        ...
        emit(
            ...
            {"status": "unsubscribed", "simulation_id": simulation_id},
        )
    ...
    while True:
        try:
            ...
            if result:
                ...
                logger.info(
                    f"Received update: {update_type} for simulation {simulation_id}"
                )
